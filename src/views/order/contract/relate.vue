<template>
  <div class="page-container">
    ...
    <RelateOrderProduct
      ...
      :order-id="contractData.id"
      :page-fn="assignProductPageList"
      :assign-fn="assignContractProduct"
      @submit="requestRelateProductPageList"
    />

    <EditOrderProduct
      :visible.sync="dialogPriceVisible"
      :order-data="contractData"
      :product-data="productData"
      :update-fn="updateContractProduct"
      @submit="requestRelateProductPageList"
    />
  </div>
</template>
...
import { assignProductPageList, relateProductPageList, assignContractProduct, updateContractProduct, deleteContractProduct } from '@/api/contract'
...
export default {
  ...
  data() {
    return {
      ...
      productData: {},
      authData: {
        isAdd: ['contract:add', 'contract:edit'],
        isEdit: ['contract:add', 'contract:edit'],
        isDelete: ['contract:add', 'contract:edit']
      }
    }
  },
  async created() {
  ...
  methods: {
    ...
    deleteCommonFn({ productId, name }) {
      const { id: contractId, sn: contractSn } = this.contractData
      this.$confirm('确认删除合同单【' + contractSn + '】的货品【' + name + '】吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async() => {
        const { msg } = await deleteContractProduct({
          contractId,
          productId: productId
        })
