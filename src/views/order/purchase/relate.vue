  ...
  <div class="page-container">
    ...
    <div class="table-page-wrap">
      <div class="table-action clearfix">
        <el-button type="text">当前采购单编号：{{ purchaseData.sn }}</el-button>
    ...
    <RelateOrderProduct
      ...
      :assign-fn="assignPurchaseProduct"
      @submit="requestRelateProductPageList"
    />

    <EditOrderProduct
      :visible.sync="dialogPriceVisible"
      :order-data="purchaseData"
      :product-data="productData"
      :update-fn="updatePurchaseProduct"
      @submit="requestRelateProductPageList"
    />
  </div>
</template>
...
import { assignProductPageList, relateProductPageList, assignPurchaseProduct, updatePurchaseProduct, deletePurchaseProduct } from '@/api/purchase'
...
export default {
  ...
  data() {
    return {
      ...
      productData: {},
      authData: {
        isAdd: ['purchase:add', 'purchase:edit'],
        isEdit: ['purchase:add', 'purchase:edit'],
        isDelete: ['purchase:add', 'purchase:edit']
      }
    }
  },
  async created() {
    ...
    if (!id) {
      return this.$message.warning('缺少采购单id参数')
    }
    this.purchaseData = {
  ...
  methods: {
    ...
    deleteCommonFn({ productId, name }) {
      const { id: purchaseId, sn: purchaseSn } = this.purchaseData
      this.$confirm('确认删除采购单【' + purchaseSn + '】的货品【' + name + '】吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async() => {
        const { msg } = await deletePurchaseProduct({
          purchaseId,
          productId: productId
        })
