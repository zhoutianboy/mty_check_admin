        ...
        >
          ...
          >
            <template slot-scope="scope">
              <PriceInput :price.sync="multipleSelection[scope.$index].supplierPrice" :width="dialogFormWidth.value" />
            </template>
          </el-table-column>
        </el-table>
      </div>
...
import { assignProductPageList, assignSupplierProduct } from '@/api/supplier'

export default {
  name: 'ProvideProductDialog',
  components: {
    PriceInput
  },
  props: {
  ...
  methods: {
    ...
    async requestProductPageList() {
      try {
        ...
        const { data } = await assignProductPageList({
          id: this.supplierId,
          ...this.pageData,
          ...this.searchForm
        })
        const { list, total } = data
        this.tableData = list.map(item => ({ ...item, supplierPrice: '0.00' }))
    ...
    async handleDialogSubmitAction(formName) {
      ...
      try {
        const productList = this.multipleSelection.map(item => ({ id: item.id, price: item.supplierPrice }))
        const { msg } = await assignSupplierProduct({
