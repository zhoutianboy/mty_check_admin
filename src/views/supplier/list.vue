      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            <el-button v-if="authData.isProvide" type="warning" @click="handleTableProvideAction(scope.$index, scope.row)">供应货品</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
...
import { supplierPageList, addSupplier, deleteSupplier, updateSupplier } from '@/api/supplier'
...
export default {
  ...
  data() {
    return {
      ...
      authData: {
        isAdd: 'supplier:add',
        isEdit: 'supplier:edit',
        isDelete: 'supplier:delete',
        isProvide: 'supplier:provide'
      },
      tempFileUrl: ''
    }
  },
  computed: {
    ...
    initDialogTitle() {
      return !this.dialogForm.id ? '新增供应商' : '编辑供应商'
    }
  },
  async created() {
    // 获取按钮权限
    ...
    await this.requestSupplierPageList()
  },
  methods: {
    ...mapActions('user', [
      'checkActionList'
    ...
    async requestSupplierPageList() {
      try {
        this.tableLoading = true
        const { data } = await supplierPageList({ ...this.pageData, ...this.searchForm })
    ...
    handleTableProvideAction(index, item) {
      this.$router.push('/supplier/provide?id=' + item.id + '&name=' + item.name)
    },
    deleteCommonFn({ id, name }) {
      this.$confirm('确认删除供应商【' + name + '】吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async() => {
        const { msg } = await deleteSupplier(id)
    ...
    handleDialogSubmitAction(formName) {
      ...
      this.$refs[formName].validate(async valid => {
        if (valid) {
          const requestName = !this.dialogForm.id ? addSupplier : updateSupplier
          try {
